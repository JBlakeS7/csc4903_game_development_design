...
{
    const float durationDefault = 2f;
    static FadeEffect main;
    private void Awake()
    {
    ...
    public static void FadeCanvasIn(CanvasGroup canvas, float duration = durationDefault)
    {
        main.StartCoroutine(FadeCanvas(canvas, canvas.alpha, 1, duration));
    }

    public static void FadeCanvasOut(CanvasGroup canvas, float duration = durationDefault)
    {
        main.StartCoroutine(FadeCanvas(canvas, canvas.alpha, 0, duration));
    }
